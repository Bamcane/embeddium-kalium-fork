...
public class BakedQuadGroupAnalyzer {
    ...
    private SpriteTransparencyLevel getQuadTransparencyLevel(BakedQuadView quad) {
        if ((quad.getFlags() & ModelQuadFlags.IS_PASS_OPTIMIZABLE) == 0 || quad.kalium$getSprite() == null) {
            return SpriteTransparencyLevel.TRANSLUCENT;
        }

        return SpriteTransparencyLevel.Holder.getTransparencyLevel(quad.kalium$getSprite());
    }
    ...
    public static Material chooseOptimalMaterial(int analyzerFlags, Material defaultMaterial, RenderPassConfiguration<?> renderPassConfiguration, BakedQuadView quad) {
        if (defaultMaterial == renderPassConfiguration.defaultSolidMaterial() || (analyzerFlags & USE_RENDER_PASS_OPTIMIZATION) == 0 || (quad.getFlags() & ModelQuadFlags.IS_PASS_OPTIMIZABLE) == 0 || quad.kalium$getSprite() == null) {
            // No improvement possible
            return defaultMaterial;
        }

        SpriteTransparencyLevel level = SpriteTransparencyLevel.Holder.getTransparencyLevel(quad.kalium$getSprite());
