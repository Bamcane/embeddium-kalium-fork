...
public class SpecialBlockRenderer {
    ...
    public void voxelizeLightBlock(BlockAndTintGetter world, BlockPos relativeBlockPos, BlockState blockState, ChunkBuildBuffers buffers) {
        ...
        if (buffers.get(material).getEncoder() instanceof ContextAwareChunkVertexEncoder encoder) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                vertices[i].v = 0;
                vertices[i].color = 0;
                vertices[i].light = WorldUtil.getLightEmission(blockState, world, relativeBlockPos) << 4 | WorldUtil.getLightEmission(blockState, world, relativeBlockPos) << 20;
            }
